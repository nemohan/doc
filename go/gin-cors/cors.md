...
cors 是gin的一个中间件，项目很小只包含三个文件config.go、cors.go、utils.go。

cors可以解决的问题：

* 在response 头部中添加控制跨域访问的头部，Access-Control-Allow-Origin。既支持精确匹配也支持通配符的方式
...
* 处理OPTIONS，在Response头部中添加允许客户端使用的请求方法Access-Control-Allow-Methods
...
type cors struct {
	...
	allowOrigins     []string
	exposeHeaders    []string
	normalHeaders    http.Header //在非OPTIONS请求时，会添加到Response中的头部
	preflightHeaders http.Header //在OPTIONS请求中，会添加到Response中的头部
...
func (cors *cors) applyCors(c *gin.Context) {
    ...
    //处理OPTIONS方法。cors会根据当前的配置动态的添加一些头部信息在response header中
    ...
    //设置了allowAllOrigins为True的话，cors也不会在respons的header中设置
    //Access-Control-Allow-Origin: *
    //但是会在上面的handlePreflight 或cors.handleNormal中设置
