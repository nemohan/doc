...
今天跟同事聊起标记-压缩算法，只记得基本思想是重新移动对象。具体的步骤没什么印象了(细节没跟同事解释清除,看来还是没有深刻理解)。假如让我实现该算法我该怎么做呢？所以尝试实现算法理清楚主要的逻辑应该会加深我对此的理解
...
3 从根开始，遍历活动对象，用对象的新位置更新对象之间的引用。Lisp2算法的这一步是顺序扫描堆，更新对象的引用关系
...
def update():
	for r in roots:
		r = update_reference(r)
def update_reference(o):
	if o.updated == False:
		o.updated = True
		for c in children(o):
			o.c = update_reference(c)
	return r.forwarded
...
def move():
	...
	while scan < heap_end:
		if scan.marked == True:
			...
			//scan.updated = False
			scan.forwarded = None
			copy_data(scan.forwarded, scan, scan.size)
		scan += scan.size
...
def compact():
	free = heap_start
	scan = heap_end - OBJ_SIZE
	while  free < scan:
		while new_address.marked == True:
			fee += free.size
