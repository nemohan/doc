...
目前来看词法扫描这部分已经都掌握了，且能熟练的使用代码实现相关功能。
...
* **上下文无关语法的BNF及EBNF表示**
* 自顶向下解析方法(top-dwon)。包括recursive-descent(递归下降解析)、LL(1)、LL(K)解析
* **自底向上解析(down-top)。LR()**

目前只掌握了上下文无关语法以及用`递归下降方法`实现语法解析。用什么样的数据结构表示AST（抽象语法树）也一知半解

### BNF、EBNF

* 语法歧义。一个语法规则可以产生多棵不同的语法树(parse tree) 。常见于操作符关联性
* 操作符优先级及关联性。左递归(left recursive)可以解决关联性的问题
...
![1607741906512](${img}/1607741906512.png)
...
* 属性规则(attribute grammer)。对每个语法规则 X<sub>0</sub> -> X<sub>1</sub>X<sub>2</sub> ...X<sub>n</sub>。  对其中的每个语法符号X的属性a有如下规则： X<sub>i</sub>.a<sub>j</sub> = f<sub>i</sub><sub>j</sub>(X<sub>0</sub>.a<sub>1</sub>，...，X<sub>0</sub>.a<sub>k</sub>，X<sub>1</sub>.a<sub>1</sub>，...，X<sub>1</sub>.a<sub>k</sub>，X<sub>n</sub>.a<sub>1</sub>，...，X<sub>n</sub>.a<sub>k</sub>)
* 继承属性(inherited attribute)。属性规则右边的属性从左边继承而来
* 综合属性(synthesize attribute)。即属性规则左边依赖右边
* 符号表(symbol table)。构建符号表的时机，解析过程中就开始构建么。构建时机选择依赖的因素?
* 类型检查



6.3.3小节提到，像C语言这种要求`变量声明在使用之前(declaration before use)`可以一趟编译完成
...
* 什么是一趟编译器(one pass compiler)，什么是多趟编译器。具备哪些条件可以一趟完成编译
...
1.  构造词法扫描器
2. 先用BNF表述语言的语法，然后将BNF形式转换为EBNF形式。依据EBNF使用Recursive-descent方法构造解析器
3. 
...
compound-stmt = "{" local-declarations statementt-lit "}" .
...
iteration-stmt = "while" "(" expression ")" .

return-stmt = "return" ; | "return" exression ; .
