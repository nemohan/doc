...
结合索引文件的格式，看一下其设计是否合理和这样设计的原因。首先来看index文件的最后52个字节。这块空间用来存放symbol table、series  table等在索引文件中的位置。如此一来，可以快速定位到索引文件中各个模块的入口位置。那这些表的作用是啥



##### symbol table:

存放所有 label name、及label value

##### series table:

##### label index table: 

存放label name，及name对应的所有value在索引文件中的位置（value在symbol table中的位置)。

若查找具有某个label name的所有label value将会很方便



##### postings table:

存放label value 所在的series 在series table中的位置。



##### label index offset table:

存放label name(name的长度和name字符串)，以及name在`label index table`中的位置。

<font color="red">估计的查找过程：先从TOC table 找到`label index offset table`的位置。将`label index offset table`加载到内存，根据`label index offset table`确定name 在`label index table`中的位置。将对应的label index 数据加载到内存，然后根据所有的value的位置信息，找到所有的value。**那么还差一步，就是 value所在的所有series**。</font>此时就该 posting offset table 上场了，根据`posting offset table`可以确定value在 `posting table`中的位置。而posting table 又存储了value所在的series在`series table`中的位置.



##### posting offset table:

存放label name(name的长度和name字符串)、name对应的value以及value在posting table 中的位置。
...
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
symbol table在文件中的位置 | series table在文件中的位置
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 label index 的位置 | label_index offset table 的位置
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
posting在文件中的位置 
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 posting offset table 在文件中的位置  
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
crc32
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
symbol table 格式(已经排好序):
symbol是label的key或value
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
长度 包括symbol个数,但不包括自身(4字节) | symbol 个数(4字节)
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
symbol 的长度| symbol 
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
symbol_1的长度| symbol_1
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
.....
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
symbol 的长度| symbol 
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
校验和 校验和包括symbol个数和数据 (4字节)
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
====================================================================
label index table格式:
label name 对应的label values在index文件中symbol table部分的位置
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
长度 | name个数 1个 | values个数 | value1 在索引文件中的位置| value2 在文件中的位置 |crc32
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
长度 | name个数 1个 | values个数 | value1 在索引文件中的位置| value2 在文件中的位置 |crc32
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
postings table格式:
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
数据长度 | off 个数 | value1 所在的series 在文件中的位置 |value2 所在的series在index文件的位置| crc32
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 数据长度 | off 个数 | value1 所在的series 在文件中的位置 |value2 所在的series在index文件的位置 |crc32
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
....
...
数据长度 
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
label的个数｜label name的长度 | name字符串  | label_index记录在文件中的位置 
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
数据长度	| 总的记录数目 
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
key个数 |name 长度 |	name | value 长度	|value |对应的postings table 记录在文件位置
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
3.  将label index offset table 部分数据读入内存。
4.  
...
func (w *Writer) writePostingsToTmpFiles() error {
    ...
    //labelNames是map[string]uint64类型
    ...
    //将还在应用内缓存的数据刷到系统缓存或磁盘,这样将f映射到内存时，才是全部内容
	if err := w.f.Flush(); err != nil {
		return err
	}
    //文件映射到内存，方便解析
